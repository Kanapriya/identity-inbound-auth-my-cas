...
import org.wso2.carbon.identity.sso.cas.internal.CASAuthenticatorServiceComponent;
...
public class HttpSAMLResponseFactory extends HttpIdentityResponseFactory {
    ...
    private HttpIdentityResponse.HttpIdentityResponseBuilder sendResponse(IdentityResponse identityResponse) {
        ...
        if (CASAuthenticatorServiceComponent.getSsoRedirectHtml() != null) {
    ...
    private String getRedirectHtml(String acUrl, String relayState, String authenticatedIdPs, SAMLLoginResponse
        ...
        String htmlPage = CASAuthenticatorServiceComponent.getSsoRedirectHtml();
    ...
    private HttpIdentityResponse.HttpIdentityResponseBuilder sendNotification(IdentityResponse identityResponse) {
        ...
        try {
            queryParams.put(CASSSOConstants.STATUS, new String[]{URLEncoder.encode(errorResponse.getStatus(),
                    StandardCharsets.UTF_8.name())});
            queryParams.put(CASSSOConstants.STATUS_MSG, new String[]{URLEncoder.encode(errorResponse.getMessageLog()
                    , StandardCharsets.UTF_8.name())});

            if (StringUtils.isNotEmpty(errorResponse.getErrorResponse())) {
                queryParams.put(CASSSOConstants.SAML_RESP, new String[]{URLEncoder.encode(errorResponse
                        .getErrorResponse(), StandardCharsets.UTF_8.name())});
            }

            if (StringUtils.isNotEmpty(errorResponse.getAcsUrl())) {
                queryParams.put(CASSSOConstants.SERVICE, new String[]{URLEncoder.encode(errorResponse
